...
import { rescorePool } from './projections.js';
...
export function buildEvent(event, options = {}) {
  ...
  const roster = reconcileGame(players).map((player) => rescore(player));
...
/**
 * Build recommendations for one site.
 *
 * The pool is re-scored first, because projections are a forecast of the
 * football game and the sites pay differently for the same forecast. Salaries
 * are then attached from that site's own export, or estimated against that
 * site's cap — mixing the two would put DraftKings prices under a FanDuel cap
 * and produce lineups $10,000 light.
 *
 * Kickers are dropped: neither site's classic roster has a kicker slot, and
 * leaving them in only lets one take a flex spot it can never fill.
 */
export function buildSiteDfs(board, site, salaryRows = [], options = {}) {
  const { count = 3 } = options;

  const eligible = board.players.filter((p) => p.position !== 'K' && p.position !== 'FLEX');
  const priced = attachSalaries(rescorePool(eligible, site), salaryRows, site.id);
  ...
  for (const slate of slates) {
    ...
    const minimum = slate.format === 'showdown' ? 8 : 30;
    ...
    // slot with someone who is not a tight end produces a lineup the site will
    // not accept, so the slate is skipped and said so rather than built wrong.
    if (slate.format === 'classic' && !pool.some((p) => p.position === 'TE')) {
      skipped.push({ name: slate.name, reason: `no tight ends identified — add a ${site.name} salary export for real positions` });
      continue;
    }

    recommendations.push(recommendForSlate(pool, slate, { count, site }));
  }

  return { site: site.id, siteName: site.name, salaryCap: site.salaryCap, salarySource, slates: recommendations, skipped };
}

/**
 * Build recommendations for every site.
 *
 * `salariesBySite` maps a site id to whatever export was found for it. A site
 * with no export still gets lineups, built on estimated salaries and flagged as
 * such — the shapes are useful even when the prices are not exact.
 */
export function buildDfs(board, salariesBySite = {}, options = {}) {
  const { count = 3, demo = false } = options;

  const sites = {};
  for (const [id, site] of Object.entries(SITES)) {
    sites[id] = buildSiteDfs(board, site, salariesBySite[id] ?? [], { count });
  }

  return { generatedAt: new Date().toISOString(), demo, sites };
}
