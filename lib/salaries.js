 ...
 *
 * Rounded to the nearest hundred because DraftKings salaries are, and a lineup
 * summing to $49,987 would give away that the numbers are not real ones.
 */
export function estimateSalary(position, points) {
  const model = SALARY_MODEL[position] ?? SALARY_MODEL.WR;
 ...
 */
export function attachSalaries(players, salaryRows = []) {
  if (!salaryRows.length) {
    return players.map((player) => ({
      ...player,
      salary: estimateSalary(player.position, player.points),
