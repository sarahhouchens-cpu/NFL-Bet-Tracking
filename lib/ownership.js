...
/**
 * Single-game seats, spread across positions rather than required at any of
 * them. DraftKings rosters six and FanDuel five, so the shares differ: the same
 * player is meaningfully more owned on the smaller roster, because there are
 * fewer places for the field to hide.
 */
export const SHOWDOWN_SEATS = {
  draftkings: { QB: 1, RB: 1.4, WR: 2, TE: 0.8, DST: 0.4, K: 0.4 },
  fanduel: { QB: 0.9, RB: 1.2, WR: 1.7, TE: 0.7, DST: 0.3, K: 0.2 },
};

/** Seat shares for a site and slate format. */
export function seatsFor(site, format) {
  if (format !== 'showdown') return CLASSIC_SEATS;
  return SHOWDOWN_SEATS[site?.id] ?? SHOWDOWN_SEATS.draftkings;
}
