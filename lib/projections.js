...
export function inferPosition(stats) {
  ...
  if (stats.has('carries') && !stats.has('receptions')) return 'RB';
  if (stats.has('carries') && stats.has('receptions')) {
    // Backs are priced for carries first; pass catchers who take a handoff are
    // priced for receptions first. Compare the two volumes.
    return (stats.get('carries') ?? 0) >= (stats.get('receptions') ?? 0) ? 'RB' : 'WR';
  }
  if (stats.has('receptions') || stats.has('recYards')) return 'WR';
  return 'FLEX';
}
...
export function projectPlayer(playerName, quotes, context = {}) {
  const { gameTouchdownScale = 1, declaredPosition = null } = context;
  ...
  const points = projectedPoints(projection, bonuses);
  ...
  return {
    ...
    points,
    floor: pointsAtQuantile(projection, position, FLOOR_QUANTILE),
    ceiling: pointsAtQuantile(projection, position, CEILING_QUANTILE),
    lines,
  };
}
 ...
 * beside them.
 */
export function rescore(player) {
  ...
  return {
    ...player,
    bonuses,
    points: projectedPoints(projection, bonuses),
    floor: pointsAtQuantile(projection, position, FLOOR_QUANTILE),
    ceiling: pointsAtQuantile(projection, position, CEILING_QUANTILE),
  };
}
 ...
 * support.
 */
export function pointsAtQuantile(projection, position, quantile) {
  ...
  return projectedPoints(shifted, bonuses);
}
