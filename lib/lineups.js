...
/**
 * What a player costs in a given slot.
 *
 * A DraftKings showdown captain scores 1.5x and *costs* 1.5x. Counting only the
 * points multiplier while charging the flex price lets the optimiser build a
 * lineup that looks affordable here and is rejected at the DraftKings cap —
 * which is the worst kind of wrong, because it is only discovered when you try
 * to enter it.
 *
 * A real salary export lists the captain as its own row at the raised price;
 * attachSalaries deliberately keeps the lower flex figure so the multiplier is
 * applied in exactly one place, here.
 */
export function slotSalary(player, slot) {
  return player.salary * (slot.multiplier ?? 1);
}
...
export function fillRoster(roster, pool, options = {}) {
  ...
  for (let i = openSlots.length - 1; i >= 0; i--) {
    const cheapest = bySlot[i].length
      ? Math.min(...bySlot[i].map((p) => slotSalary(p, openSlots[i].slot)))
      : Infinity;
  ...
  for (const [index, player] of lockedSlots) {
    ...
    seedSalary += slotSalary(player, slot);
  ...
  for (let position = 0; position < openSlots.length; position++) {
    ...
    for (const state of states) {
      ...
      for (let i = startFrom; i < candidates.length; i++) {
        ...
        const salary = state.salary + slotSalary(player, slot);
...
function summarize(players, contest) {
  const salary = players.reduce((sum, p) => sum + p.salary * (p.multiplier ?? 1), 0);
