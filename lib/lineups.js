...
import { DEFAULT_SITE, rosterFor, slotSalary } from './sites.js';
import { projectOwnership, leverage, seatsFor } from './ownership.js';
...
export function fillRoster(roster, pool, options = {}) {
  ...
  for (let i = openSlots.length - 1; i >= 0; i--) {
    const cheapest = bySlot[i].length
      ? Math.min(...bySlot[i].map((p) => slotSalary(p, openSlots[i].slot, site)))
      : Infinity;
  ...
  for (const [index, player] of lockedSlots) {
    ...
    seedSalary += slotSalary(player, slot, site);
  ...
  for (let position = 0; position < openSlots.length; position++) {
    ...
    for (const state of states) {
      ...
      for (let i = startFrom; i < candidates.length; i++) {
        ...
        const salary = state.salary + slotSalary(player, slot, site);
  ...
  return states.slice(0, collect).map((state) => summarize(state.players, contest, site));
}
...
function summarize(players, contest, site = DEFAULT_SITE) {
  const salary = players.reduce((sum, p) => sum + slotSalaryOf(p, site), 0);
  ...
  return {
    ...
    players,
    site: site.id,
    // Lets the page show each roster row at what it actually costs, which is
    // not the list price for a DraftKings captain.
    chargesCaptainPremium: Boolean(site.showdown.captainCostsMultiplier),
    salary,
    salaryRemaining: site.salaryCap - salary,
    points: range.mean,
    floor: range.floor,
...
/**
 * Both lineup styles for one slate, on one site.
 *
 * Ownership is projected once per slate and shared between the two contest
 * types, because it is a property of the slate's player pool rather than of the
 * contest you enter. It is *not* shared between sites: the pools are priced
 * differently and scored differently, so the same player is chalk on one and
 * ignored on the other.
 */
export function recommendForSlate(players, slate, options = {}) {
  const { count = 3, site = DEFAULT_SITE } = options;
  const showdown = slate?.format === 'showdown';
  const roster = rosterFor(site, showdown ? 'showdown' : 'classic');

  const pool = projectOwnership(players, { seats: seatsFor(site, showdown ? 'showdown' : 'classic') })
  ...
  const tournament = showdown
    ? selectDiverse(
        fillRoster(roster, pool, { contest: 'tournament', site, rules: rules.tournament, collect: 300, beamWidth: 400 }),
        { count, rules: rules.tournament, sortKey: 'ceiling' }
      )
    : buildTournamentLineups(pool, { count, site, roster, rules: rules.tournament });

  return {
    site: site.id,
    siteName: site.name,
    slate: { id: slate.id, name: slate.name, format: slate.format, startsAt: slate.startsAt },
    captainLabel: showdown ? site.showdown.captainLabel : null,
    salaryCap: site.salaryCap,
    poolSize: pool.length,
    tournament,
    doubleUp: buildDoubleUpLineups(pool, { count, site, roster, rules: rules.double_up }),
  };
}

/**
 * Cost of a player already placed in a slot.
 *
 * The multiplier travels on the player once slotted, so this mirrors
 * slotSalary() for a player that has already been assigned — and, like it,
 * charges the premium only where the site actually charges one.
 */
function slotSalaryOf(player, site) {
